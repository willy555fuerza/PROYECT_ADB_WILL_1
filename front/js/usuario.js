...
formAgregarUsuario.addEventListener("submit", async function(event) {
    ...
    const usuario = document.getElementById('usuario').value; // Nuevo campo de perfil
    const contraseña = document.getElementById('contraseña').value;
 

   // Agregar el archivo de la foto al objeto FormData

    try {
        // Enviar los datos al servidor para crear el nuevo usuario
        const response = await fetch('http://localhost:3009/ADB/create_user', {
            method: 'POST',
            headers: {
